...
@property (weak, nonatomic) UITapGestureRecognizer *dimmingViewTapGesture;
@property (nonatomic) CGFloat bottomHeight;
@property (nonatomic) ISHPullUpState stateAtStartOfGesture;
@property (weak, nonatomic) ISHPullUpDimmingView *dimmingView;
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture
{
    ...
    if (gesture.state == (UIGestureRecognizerStateChanged | UIGestureRecognizerStateEnded))
    {
        ...
        self.bottomControllerVelocityAtSwipeOfGesture = velocity.y;
    }
    ...
    switch (gesture.state)
    {
        ...
        case UIGestureRecognizerStateFailed:
        {
            ...
            {
                if ([self.sizingDelegate respondsToSelector:@selector(pullUpViewController:targetHeightForBottomViewController:fromCurrentHeight:)])
                {
...
- (CGFloat)minimumBottomHeight
{
    ...
    if ([self.sizingDelegate respondsToSelector:@selector(pullUpViewController:minimumHeightForBottomViewController:)])
    {
...
- (CGFloat)maximumBottomHeightWithSize:(CGSize)size
{
    ...
    if ([self.sizingDelegate respondsToSelector:@selector(pullUpViewController:maximumHeightForBottomViewController:maximumAvailableHeight:)])
    {
...
- (void)updateViewLayoutBottomHeight:(CGFloat)bottomHeight withSize:(CGSize)size
{
    ...
    switch (self.bottomLayoutMode)
    {
        case ISHPullUpBottomLayoutModeShift:
        {
            ...
            bottomFrame = CGRectMake(0, yPosition, bounds.size.width, expandedBottomHeight);
            break;
        }
        ...
        case ISHPullUpBottomLayoutModeResize:
        {
            ...
            bottomFrame = CGRectMake(0, yPosition, bounds.size.width, clampedBottomHeight);
            break;
        }
    }
    ...
    if (self.contentViewController)
    {
        ...
        if ([self.contentDelegate respondsToSelector:@selector(pullUpViewController:updateEdgeInsets:forContentViewController:)])
        {
    ...
    {
        if ([self.sizingDelegate respondsToSelector:@selector(pullUpViewController:updateEdgeInsets:forBottomViewController:)])
        {
